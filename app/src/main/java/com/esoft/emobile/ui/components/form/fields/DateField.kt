...
class DateField(
    ...
    override fun Field() {
        ...
        val date = if (value.value != null) LocalDate.parse(value.value.toString(), dateFormat) else LocalDate.now()

        val datePickerDialog = DatePickerDialog(
            context,
            { _, year, month, dayOfMonth ->
                this.onChange(LocalDate.of(year, month + 1, dayOfMonth), form)
            },
            date.year,
            date.monthValue - 1,
            date.dayOfMonth,
